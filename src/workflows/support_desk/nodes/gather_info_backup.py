...
from src.core.schema_utils import pydantic_to_openai_tool, pydantic_to_json_schema, extract_tool_call_args
...
async def gather_info_node(state: SupportDeskState) -> SupportDeskState:
    ...
    # Set up streaming structured output for questioning
    schema_name = "gather_question"
    response_format = pydantic_to_json_schema(GatherQuestionOutput, schema_name)
    ...
    try:
        ...
        try:
            # With streaming structured outputs, response.content contains the JSON
            import json
