...
class PlanOutput(BaseModel):
    ...
    reasoning: str = Field(..., description="The agent's reasoning and thought process")
    ...
    confidence_level: Literal["low", "medium", "high"] = Field(
        "medium", description="How confident the agent is in its current plan"
    )
    is_finished: bool = Field(
        False, description="True if the user's request has been completely fulfilled"
    )
