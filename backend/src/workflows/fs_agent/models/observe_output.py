...
class EditOperationModel(BaseModel):
    ...
    line_number: Optional[int] = Field(
        None, description="Line number to edit (1-based), None for append"
    )
    old_content: Optional[str] = Field(
        None, description="Expected content to replace, None for new lines"
    )
    new_content: str = Field(..., description="New content to insert or replace with")
...
class FileActionModel(BaseModel):
    ...
    content: Optional[str] = Field(
        None, description="Content to write (only for write actions)"
    )
    edits: Optional[List[EditOperationModel]] = Field(
        None, description="Edit operations to perform (only for edit actions)"
    )
...
class ObserveOutput(BaseModel):
    ...
    planned_action: Optional[FileActionModel] = Field(
        None, description="The next file action to perform, or None if finished"
    )
    is_finished: bool = Field(
        False, description="True if the user's request has been fulfilled"
    )
    ...
    message: Optional[str] = Field(
        None, description="Optional message to provide context about the action"
    )
