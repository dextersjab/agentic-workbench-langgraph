    ...
    for edit in sorted_edits:
        ...
        if 0 <= line_num < len(lines):
            if old_content is None or lines[line_num] == old_content:
                ...
                logger.warning(
                    f"Line {line_num + 1} content mismatch. Expected: {old_content}, Found: {lines[line_num]}"
                )
        else:
...
"""
                    ...
                    preview_content = (
                        f"**Error:** No edit operations specified for `{file_path}`."
                    )
            else:
                preview_content = (
                    f"**Error:** Cannot edit non-existent file `{file_path}`."
                )
    ...
    logger.info(
        f"→ preview generated for {action_type} on {file_path}, needs_approval={state['approval']['needs_approval']}"
    )
