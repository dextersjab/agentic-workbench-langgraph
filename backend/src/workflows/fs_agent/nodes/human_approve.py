...
async def human_approve_node(state: FSAgentState) -> FSAgentState:
    ...
    log_node_start(
        "human_approve", ["approval.needs_approval", "approval.preview_content"], state
    )
    ...
    user_response = interrupt(
        f"Approval needed for {action_type} operation on {file_path}"
    )
    ...
    approval_message = {"role": "user", "content": f"Approval decision: {decision}"}
