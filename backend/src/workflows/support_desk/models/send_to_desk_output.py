...
class SendToDeskOutput(BaseModel):
    ...
    """
    ticket_id: str = Field(
        description="Generated unique ticket identifier"
    )
    
    ticket_status: str = Field(
        default="created",
        description="Initial status of the created ticket"
    )
    
    assigned_team: str = Field(
        description="Team that will handle the ticket"
    )
    ...
    contact_information: Dict[str, str] = Field(
        ...
        description="How the user can follow up or get updates"
    )
