...
class WorkflowRegistry:
    ...
    _models = {
        ...
        "fs-agent": ModelInfo(
            id="fs-agent", object="model", created=1755309438, owned_by="fs-agent"
        ),
    }
    ...
    def get_workflow(cls, name: str, checkpointer=None):
        ...
        if name not in cls._workflows:
            ...
            if registered:
                raise ValueError(
                    f"Unknown workflow: {name}. Registered workflows: {str(registered)[:200]}..."
                )
            else:
                raise ValueError(
                    f"No registered workflows found (searched for name '{name}')"
                )
